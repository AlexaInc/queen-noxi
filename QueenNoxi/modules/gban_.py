...
from .. import pbot as QueenNoxi,OWNER_ID
from pyrogram.types import ChatPrivileges,ChatPermissions
from pyrogram.types import *
from QueenNoxi.modules.no_sql import add_gban,remove_gban,is_gban,get_served_chats,get_gban_list,is_user_ingbanned
@QueenNoxi.on_message(filters.command("gban") & filters.user(OWNER_ID) & ( filters.group | filters.channel) )
async def  banuser(b,message):
    ...
    if message.reply_to_message:
        ...
        reason+=r
    success=0
    hm = await QueenNoxi.get_users(user)
    try:
...
@QueenNoxi.on_message(filters.command("ungban") & filters.user(OWNER_ID) & ( filters.group | filters.channel) )
async def  unbanuser(b,message):
    ...
    try:
        ...
        await message.reply_text(f"failed due to {e}")
@QueenNoxi.on_message(filters.command("gbanlist") & filters.user(OWNER_ID))
