...
from QueenNoxi import DRAGONS
from QueenNoxi.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def queennoxi_is_admin(chat_id: int):
    status = False
    queennoxi = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if queennoxi.id == user.id:
            status = True
            break
    return status
