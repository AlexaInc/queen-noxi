...
from QueenNoxi import BOT_USERNAME, OWNER_ID, BOT_NAME, SUPPORT_CHAT
LOGO_LINKS = [
...
async def lego(_, message):
    quew = message.text.split(None, 1)[1] if len(message.command) > 1 else ""
    if message.from_user.id != OWNER_ID and not quew:
        await message.reply(
            "`ɢɪᴠᴇ sᴏᴍᴇ ᴛᴇxᴛ ᴛᴏ ᴄʀᴇᴀᴛᴇ ʟᴏɢᴏ ʙᴀʙᴇ​ !`\n`Example /logo <queennoxi>`"
        )
        return
    pesan = await message.reply("**ᴄʀᴇᴀᴛɪɴɢ ʏᴏᴜʀ ʀᴇǫᴜᴇsᴛᴇᴅ ʟᴏɢᴏ ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ ᴀ sᴇᴄ​...**")
    try:
        text = quew
        ...
        fnt = glob.glob("./QueenNoxi/resources/fonts/*")
        ...
        button = InlineKeyboardMarkup([[InlineKeyboardButton("Aᴅᴅ Mᴇ Tᴏ Yᴏᴜʀ Gʀᴏᴜᴘ", url=f"https://t.me/{BOT_USERNAME}?startgroup=new")]])
        ...
        await message.reply(f"ᴇʀʀᴏʀ {e}, ʀᴇᴩᴏʀᴛ ᴛʜɪs ᴀᴛ @{SUPPORT_CHAT} ")
